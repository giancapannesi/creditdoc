/**
 * /api/email-signup POST — Sendy subscription + Supabase lead capture.
 * Ported from src/pages/api/email-signup.ts (322 LoC).
 *
 * Same-origin + honeypot + timing checks; per-signup-type list routing;
 * conditional lead_captures write.
 */
...
function sourceAllowed(
  sourcePage: string,
  config: { allowed_sources: readonly string[]; allowed_source_prefixes?: readonly string[] },
): boolean {
  if (config.allowed_sources.includes(sourcePage)) return true;
  return Boolean(
    config.allowed_source_prefixes?.some((prefix) => sourcePage.startsWith(prefix)),
  );
}
...
export async function handleEmailSignup(request: Request, env: Env): Promise<Response> {
  ...
  if (!sameOrigin(request)) return json({ ok: false, error: "Request rejected" }, 403);
  if (isRateLimited(request)) return json({ ok: false, error: "Too many requests" }, 429);
  ...
  let sendyRes: Response | Error;
  try {
    sendyRes = await fetch(SENDY_URL, {
      method: "POST",
  ...
  const accepted =
    normalized === "1" || normalized.toLowerCase().includes("already subscribed");
  if (!accepted) {
    return json({ ok: false, error: "Email signup rejected" }, 502);
  }

  const writes = { sendy_subscribers: true, lead_captures: false };
