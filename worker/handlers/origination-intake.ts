/**
 * /api/origination-intake POST — quiz-driven lead capture handler.
 * Ported from src/pages/api/origination-intake.ts (358 LoC).
 */
...
export async function handleOriginationIntake(request: Request, env: Env): Promise<Response> {
  ...
  const resultConfig = config.results[resultLabel as keyof typeof config.results] as
    { recommended_route: string; title: string };
