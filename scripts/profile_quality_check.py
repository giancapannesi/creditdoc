...
def check_profile(data):
    ...
    has_fallback = has_website  # DDG favicon fallback works if website_url exists
    results['logo'] = {
        'pass': has_logo,
        'fallback': has_fallback,
        'score': 5 if has_logo else (3 if has_fallback else 0),
        'max': 5,
        'detail': data.get('logo_url', 'Fallback via DDG' if has_fallback else 'FAIL: no logo and no website for fallback')
    }
